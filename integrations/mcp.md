...
### Example 1: MCP Server with Streamable HTTP Transport

This example demonstrates how to create a simple calculator server using MCP and connect to it using the MCPTool with streamable HTTP transport.

**Step 1: Run the MCP Server**

First, run the [server](https://github.com/deepset-ai/haystack-core-integrations/blob/main/integrations/mcp/examples/mcp_server.py) that exposes calculator functionality (addition and subtraction) via MCP:
...
In a separate terminal, run the [client](https://github.com/deepset-ai/haystack-core-integrations/blob/main/integrations/mcp/examples/mcp_client.py) that connects to the calculator server:
...
pipeline = Pipeline()
pipeline.add_component("llm", OpenAIChatGenerator(tools=[time_tool]))
...
2. Uses a default OpenAI LLM to interpret the query and decide which tool to use
